...
int cardAdventurer(struct gameState *state,  int currentPlayer,  int temphand[MAX_HAND]){
	...
	while(drawntreasure<2){
		...
		cardDrawn = state->hand[currentPlayer][state->handCount[currentPlayer]-2];//top card of hand is most recently drawn card. Bug: -2 instead of -1
...
int cardVillage(struct gameState *state,  int currentPlayer, int handPos){
	...
	drawCard(currentPlayer, state);		
	//+2 Actions
	state->numActions = state->numActions + 3; // Bug: +3 instead of +2
...
int cardEmbargo(struct gameState *state, int choice1, int currentPlayer, int handPos){
	...
	//trash card
	discardCard(handPos, currentPlayer, state, 0);	//Bug: trashFlag parameter 0 instead of 1
	return 0;
}
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      cardAdventurer(state, currentPlayer, temphand);
    ...
    case embargo: 
      cardEmbargo(state, choice1, currentPlayer, handPos);
